...
namespace small_point_lio {
    struct state {
        ...
        Eigen::Matrix<value_type, 3, 3> rotation =
            Eigen::Matrix<value_type, 3, 3>::Identity(); // 旋转
    ...
    class eskf {
    public:
        using measurement_model_point =
            std::function<void(const state&, point_measurement_result&)>;
    ...
    public:
        ...
        predict_cov(double timestamp, Eigen::Matrix<state::value_type, state::DIM, state::DIM>& Q) {
            ...
            if (dt_cov > 0) [[likely]] {
                ...
                F.block<3, 3>(state::velocity_index, state::acceleration_index) =
                    x.rotation * dt_cov;
        ...
        inline bool update_imu() {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (!measurement_result.satu_check[i]) {
                    PHT.col(i + 3) =
                        P.col(state::acceleration_index + i) + P.col(state::ba_index + i);
                    HP.row(i + 3) =
                        P.row(state::acceleration_index + i) + P.row(state::ba_index + i);
                }
            }
            for (int i = 0; i < 3; i++) {
                if (!measurement_result.satu_check[i]) {
