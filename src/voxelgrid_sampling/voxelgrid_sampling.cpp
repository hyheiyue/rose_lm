...
void quick_sort_omp_impl(
    ...
    const auto m2 =
        median3(*(first + offset * 3), *(first + offset * 4), *(first + offset * 5), comp);
    ...
    auto middle2 =
        std::partition(middle1, last, [&](const auto& val) { return !comp(pivot, val); });
...
void VoxelgridSampling::voxelgrid_sampling(
    ...
    constexpr int coord_bit_size =
        21; // Bits to represent each voxel coordinate (pack 21x3=63bits in 64bit int)
    constexpr size_t coord_bit_mask = (1 << 21) - 1; // Bit mask
    constexpr int coord_offset = 1
        << (coord_bit_size - 1); // Coordinate offset to make values positive
    ...
    for (size_t i = 0; i < points.size(); i++) {
        ...
        if ((coord < 0).any() || (coord > coord_bit_mask).any()) {
            std::cout << "voxel coord is out of range!!" << std::endl;
...
void VoxelgridSampling::voxelgrid_sampling(
    ...
    rose_lm::common::Point sum_pt = points[coord_pt[0].second];
    int voxel_count = 1;
    ...
    for (size_t i = 1; i < N; ++i) {
        ...
        bool same_voxel = (coord_pt[i].first == coord_pt[i - 1].first);
...
/// @param downsampled Downsampled points
/// @param leaf_size   Downsampling resolution
void VoxelgridSampling::voxelgrid_sampling_omp(
    const std::vector<Eigen::Vector3f>& points,
    std::vector<Eigen::Vector3f>& downsampled,
    double leaf_size,
    int num_threads
